...
1. **Test Connection**:
```bash
curl -X POST "http://localhost:8000/Dynamic_db/dbtest" \
  -H "Content-Type: application/json" \
  -d '{"host":"localhost","port":5432,"user":"postgres","password":"your_password","database":"postgres"}'
```

2. **Create Database**:
```bash
curl -X POST "http://localhost:8000/Dynamic_db/dbcreate" \
  -H "Content-Type: application/json" \
  -d '{"host":"localhost","port":5432,"user":"postgres","password":"your_password","dbname":"myapp"}'
```

3. **Create Table**:
```bash
curl -X POST "http://localhost:8000/Dynamic_db/tablecreate" \
  -H "Content-Type: application/json" \
  -d '{"host":"localhost","port":5432,"user":"postgres","password":"your_password","database":"myapp","tablename":"users","columns":[{"name":"id","type":"INT","primary":true,"auto_increment":true,"nullable":false},{"name":"name","type":"VARCHAR(100)","primary":false,"auto_increment":false,"nullable":false}]}'
```

4. **Insert Data**:
```bash
curl -X POST "http://localhost:8000/Dynamic_db/dbinsert" \
  -H "Content-Type: application/json" \
  -d '{"host":"localhost","port":5432,"user":"postgres","password":"your_password","database":"myapp","tablename":"users","data":{"name":"John Doe"}}'
```

5. **Fetch Data**:
```bash
curl -X POST "http://localhost:8000/Dynamic_db/dbfetch" \
  -H "Content-Type: application/json" \
  -d '{"host":"localhost","port":5432,"user":"postgres","password":"your_password","database":"myapp","tablename":"users"}'
```